...
getBalance = (callback) ->
  _checkAPIKey (error) ->
    ...
    request ENDPOINT + "wow/?api_key=" + APIKEY + "&a=get_balance", (error, response, body) ->
      return callback(error)  if error
...
"Bad Query"
###
withdraw = (amount, paymentAddress, callback) ->
  _checkAPIKey (error) ->
    return callback(error)  if error
    return callback("Missing amount to withdraw.")  unless amount
    return callback("Missing payment address to send to.")  unless paymentAddress
    
    # TODO there may be an issue withdrawing amounts lower than 10 doge.
    request ENDPOINT + "wow/?api_key=" + APIKEY + "&a=withdraw&amount=" + amount + "&payment_address=" + paymentAddress, (error, response, body) ->
      return callback(error)  if error
      if response.statusCode is 200
        callback null, body
      else
...
"Invalid API Key"
###
getNewAddress = (addressLabel, callback) ->
  _checkAPIKey (error) ->
    return callback(error)  if error
    apiQuery = "wow/?api_key=" + APIKEY + "&a=get_new_address"
    if addressLabel
      if validator.isAlphanumeric(addressLabel)
        apiQuery += "&address_label=" + addressLabel
      else
        return callback("Invalid address label.")
    request ENDPOINT + apiQuery, (error, response, body) ->
      return callback(error)  if error
      if response.statusCode is 200
        callback null, body
      else
...
getAddresses = (callback) ->
  _checkAPIKey (error) ->
    return callback(error)  if error
    request ENDPOINT + "wow/?api_key=" + APIKEY + "&a=get_my_addresses", (error, response, body) ->
      return callback(error)  if error
      if response.statusCode is 200
        callback null, body
      else
...
"Invalid API Key"
###
getAddressReceived = (paymentAddress, addressLabel, callback) ->
  _checkAPIKey (error) ->
    return callback(error)  if error
    return callback("Missing payment address or address label.")  unless paymentAddress
    
    # Verify we have a read address
    _verifyAddress paymentAddress, (error) ->
      return callback(error)  if error
      
      # If we did not get an address label, then we only have payment address and callback
      addressLabel = (if typeof (addressLabel) is "undefined" then null else addressLabel)
      unless addressLabel
...
null
###
getAddressByLabel = (addressLabel, callback) ->
  _checkAPIKey (error) ->
    return callback(error)  if error
    return callback("Missing address label.")  unless addressLabel
    request ENDPOINT + "wow/?api_key=" + APIKEY + "&a=get_address_by_label&address_label=" + addressLabel, (error, response, body) ->
      return callback(error)  if error
      if response.statusCode is 200
        callback null, body
      else
...
getDifficulty = (callback) ->
  request ENDPOINT + "wow/?a=get_difficulty", (error, response, body) ->
    return callback(error)  if error
    if response.statusCode is 200
      callback null, body
    else
...
getCurrentBlock = (callback) ->
  request ENDPOINT + "wow/?a=get_current_block", (error, response, body) ->
    return callback(error)  if error
    if response.statusCode is 200
      callback null, body
    else
...
"Invalid Amount"
###
getCurrentPrice = (conversionType, amount, callback) ->
  apiQuery = "wow/?a=get_current_price"
  args = []
  argCounter = 0

  while argCounter < arguments_.length
    args.push arguments_[argCounter]
    argCounter++
  callback = args.pop()
  if args.length > 0 and typeof (args[0]) is "number"
    amount = args.shift()
...
_verifyAddress = (dogeAddr, callback) ->
  return callback("Invalid doge address.")  if dogeAddr.length isnt 34 or dogeAddr[0] isnt "D"
  _dogeChainVerify dogeAddr, (error) ->
    if error and error.code is "ETIMEOUT"
      return callback() # We could not query dogechain so we will allow DogeAPI to deal with more verification
    else return callback(error)  if error
    callback()
...
_dogeChainVerify = (dogeAddr, callback) ->
  request "http://dogechain.info/chain/Dogecoin/q/checkaddress/" + dogeAddr, (error, response, body) ->
    return callback(error)  if error
    if response.statusCode is 200
      switch body
        when "X5", "SZ", "CK"
          callback "Invalid doge address."
        else
          callback()
...
_checkAPIKey = (callback) ->
  return callback("Missing API key.")  if APIKEY is `undefined` or APIKEY is "" or APIKEY is null
  callback()
